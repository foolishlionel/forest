...
public extension DateComponents {
    ...
    static func + (left: DateComponents, right: DateComponents) -> DateComponents {
        ...
        if let minute = right.minute {
            dateComponents.month = (dateComponents.minute ?? 0) + minute
        }
    ...
    /// Create a `String` instance representing the receiver formatted in given units style.
    ///
    ...
    func string(in unitsStyle: DateComponentsFormatter.UnitsStyle) -> String? {
        let dateComponentsFromatter = DateComponentsFormatter()
        dateComponentsFromatter.unitsStyle = unitsStyle
        dateComponentsFromatter.allowedUnits = [.year, .month, .weekOfMonth, .day, .hour, .minute, .second]
        return dateComponentsFromatter.string(from: self)
    }
}
